...
// from http://dean.edwards.name/packer/
var base2 = {
  name: 'base2',
  version: '1.0.1(pre)',
  exports:
...
new (function (_) {
  ...
  var Map = Base.extend({
    ...
    has: function (a) {
      /*@cc_on@*/ /*@if(@_jscript_version<5.5)return $Legacy.has(this,_14+a);@else@*/ return _14 + a in this; /*@end@*/
    },
    merge: function (b) {
      var c = flip(this.put);
      forEach(
  ...
  var Array2 = _23(Array, Array, 'concat,join,pop,push,reverse,shift,slice,sort,splice,unshift', [
    Enumerable,
    {
  ...
  function _24(a, b, c) {
    ...
    if (typeof a == 'string') {
      ...
      for (i = 0; i < d; i++) {
        /*@cc_on@*/ /*@if(@_jscript_version<5.2)if(a[i]!==undefined||$Legacy.has(a,i))@else@*/ if (i in a) /*@end@*/ b.call(c, a[i], i, a);
      }
    }
  }
  function _8(g, h, j, k) {
  ...
  function instanceOf(a, b) {
    ...
    /*@cc_on@*/ /*@if(@_jscript_version<5.1)if($Legacy.instanceOf(a,b))return true;@else@*/ if (a instanceof b) return true;
    /*@end@*/ if (Base.ancestorOf == b.ancestorOf) return false;
    var c = a.constructor;
...
new (function (_) {
  ...
  var CSSParser = RegGrp.extend(
    {
      ...
      escape: function (b) {
        ...
        return this.optimise(
          this.format(
            String(b).replace(CSSParser.ESCAPE, function (a) {
  ...
  var XPathParser = CSSParser.extend(
    ...
    },
    {
      ...
      values: {
        identifiers: { '#': "[@id='$1'][1]", '.': "[contains(concat(' ',@class,' '),' $1 ')]" },
        combinators: {
          ' ': '/descendant::$1',
          '>': '/child::$1',
...
base2.code.Colorizer.addScheme(
  'xml',
  {
    attribute: /(\w+)=("[^"]*"|'[^']*')/,
    cdata: /<!\[CDATA\[([^\]]|\][^\]]|\]\][^>])*\]\]>/,
...
// with (base2.code.Colorizer)
base2.code.Colorizer.html.merge(base2.code.Colorizer.xml);
// with (base2.code.Colorizer)
base2.code.Colorizer.addScheme(
  'css',
  {
    at_rule: /@[\w\s]+/,
    bracketed: /\([^'\x22)]*\)/,
    comment: base2.code.Colorizer.patterns.block_comment,
    property: /(\w[\w-]*\s*):([^;}]+)/,
    special: /(\-[\w-]*\s*):/,
    selector: /([\w-:\[.#][^{};]*)\{/,
  },
...
// with (base2)
//   with (code)
base2.code.Colorizer['html-multi'] = base2.code.Colorizer.html.union({
  inline: function (a, b, c, d) {
...
base2.code.Colorizer.javascript.add(
  '\\b(' + (base2.exports + ',base,base2,merge,union,implement,Array2,Date2,String2').match(/[^\s,]+/g).join('|') + ')\\b',
  '<span class="base2">$0</span>',
);
base2.code.Colorizer.javascript.insertAt(0, /("@[^"]+"):/, '<span class="special">$1</span>:');
tabStop = 2;
// }
eval(base2.namespace);
eval(DOM.namespace);
...
var Packer = Base.extend(
  ...
  },
  {
    ...
    UNPACK:
      "eval(function(p,a,c,k,e,r){e=%5;if(!''.replace(/^/,String)){while(c--)r[%6]=k[c]" +
    ...
    init: function () {
      this.data = reduce(
        this.data,
        function (data, replacement, expression) {
